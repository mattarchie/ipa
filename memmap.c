...
void * mmap_helper(bool isheader, size_t size) {
  return NULL;
}
...
int create_header_pg(int file_no) {
  ...
  size_t chars = snprintf(&path[0], sizeof(path), "%s%d%s%d", "/tmp/bop/", getpgid(getpid()), "/headers/", file_no);
  path[chars] = '\0';
...
void allocate_header_page() {
  ...
  if (speculating()) {
    ...
    destination = ((char*) MIN((void*)shared->firstpg, shared->first_huge)) + PAGE_SIZE * mmap_page_no;
  } else {
    file_no = -1;
    destination = NULL;
  }
  file_descriptor = create_header_pg(file_no);
  headers = mmap(destination, PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, file_descriptor, 0);
  if ( (speculating() && headers != (header_page_t *) destination)
        || headers == (header_page_t *) -1) {
